...
-- =============================================
CREATE PROCEDURE [dbo].[GetHoursToEmployeeWorkByHour_Sp] 
...
BEGIN
	...
	SELECT     TOP (100) PERCENT dbo.HoursToEmployee_Ta.ReportID, MONTH(dbo.HoursToEmployee_Ta.ReportDate) AS ReportMonth, 
                      ...
                      dbo.ALL_Tasks_Ve.PlannedHours, dbo.ActionType_Ta.Description AS ActionType, dbo.ActionType_Ta.ChargingCode, 
                      dbo.HoursToEmployee_Ta.WasDrive
...
WHERE     (dbo.HoursToEmployee_Ta.Absence IS NULL) AND (dbo.ALL_Tasks_Ve.TaskStatus = 20 OR
                      dbo.ALL_Tasks_Ve.TaskStatus = 13) AND (@intReportMonth = 0) AND (@intReportYear = 0) AND (@intTaskCode = 0) OR
                      ...
                      (YEAR(dbo.HoursToEmployee_Ta.ReportDate) = @intReportYear) OR
                      (dbo.HoursToEmployee_Ta.Absence IS NULL) AND (dbo.ALL_Tasks_Ve.TaskStatus = 20 OR
                      dbo.ALL_Tasks_Ve.TaskStatus = 13) AND (@intReportMonth = 0) AND (@intReportYear = 0) AND 
                      (dbo.HoursToEmployee_Ta.TaskCode = @intTaskCode) OR
                      (dbo.HoursToEmployee_Ta.Absence IS NULL) AND (dbo.ALL_Tasks_Ve.TaskStatus = 20 OR
                      ...
                      (YEAR(dbo.HoursToEmployee_Ta.ReportDate) = @intReportYear) AND (dbo.HoursToEmployee_Ta.TaskCode = @intTaskCode)
